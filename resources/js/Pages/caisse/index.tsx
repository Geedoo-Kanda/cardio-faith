...
import FilterOperationButtons from "@/Components/FilterOperationButtons";
...
export default function Caisse({ auth, caisses }: PageProps<{ caisses: any }>) {
    ...
    const [Caisses, setCaisses] = useState<any | any[]>([]);
    const handleFilterClick = (filterValue: string) => {
        setFilter(filterValue);
    };
    const handleFilterOperationClick = (filterValue: string) => {
        setOperation(filterValue);
    };
    ...
    const fetchRdv = async () => {
        ...
        try {
            const response = await axios.get(route('caisse', currentPage), {
                params: {
                    filter: filter,
                    search: search,
                    operation: operation,
            ...
            <div className="py-12 px-4 container mx-auto">
                <div className="flex flex-wrap items-center justify-between md:px-4 mb-10">
                    <div>
                        ...
                        <span onClick={Add} className="p-4 cursor-pointer rounded-md bg-red-600 text-white mr-5 text-sm"> <BiPlusMedical className="inline-flex mr-2" />Ajouter une opération</span>
                    </div>
                    <div>
                ...
                <div className=" flex gap-2 justify-between flex-wrap items-center container mx-auto">
                    ...
                    <FilterOperationButtons filter={operation} handleFilterOperationClick={handleFilterOperationClick} />
                    ...
                    <section className="container px-4 mx-auto">
                                                ...
                                                <div className="overflow-hidden border border-gray-200  md:rounded-lg">
                                                    <table className="min-w-full divide-y divide-gray-300 ">
                                                        ...
                                                        <tbody className="bg-white divide-y divide-gray-200">
                                                            {Caisses.data.map((caisse: any, index: any) => (
                                                                <tr key={index}>
...

                                                                    {
                                                                        auth.user.roles[0].name == "Administrateur"  ?
                                                                            <td className="flex items-center justify-center px-2 h-full">
                                                                                <span onClick={() => Disable(caisse.id)} className="bg-red-600 cursor-pointer hover:bg-red-700 text-white mt-1 p-2 rounded-md text-sm">  <FaTrashAlt /></span>
                        ...
                        <div className="w-full flex justify-center">
                            <div className="pagination flex justify-center items-center gap-2 mt-4">
                                ...
                                <span className="text-sm">{`Page ${currentPage} of ${Caisses?.last_page}`}</span>
                                <button
                                    onClick={() => handlePageChange(currentPage + 1)}
                                    disabled={currentPage === Caisses?.last_page}
                                    className="px-3 py-2 bg-red-600 text-white rounded-md text-sm disabled:opacity-50 flex"
                                >
                                    Suivant
