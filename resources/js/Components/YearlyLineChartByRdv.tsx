  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white borde rounded-lg overflow-hidden shadow-lg text-sm pb-4 max-w-xs">
        ...
        <div className="px-2">
          <DataItem label="Total" value={payload[0]?.value} color="red" />
        </div>
      </div>
    );
  }
...
const YearlyLineChartByRdv = ({ monthlyData = [] }) => {
  ...
  return (
    <ResponsiveContainer width="100%" height={400}>
      <LineChart data={selected}>
        ...
        <Legend />
        <Line type="monotone" dataKey="assignments" name="Total" stroke="#ed0000" />
      </LineChart>
    </ResponsiveContainer>
  );
};

export default YearlyLineChartByRdv;
